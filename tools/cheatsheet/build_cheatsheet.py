...
SAY('SLIDE 5 → UC3 ĐĂNG KÝ TÀI KHOẢN (Huy · 95 giây)', [
 ...
 'Khách mở màn hình đăng ký, nhập email rồi bấm Gửi OTP. Nhóm em cố ý tách làm hai bước — email trước, thông tin sau — để khách không phải điền hết biểu mẫu rồi mới phát hiện email nhập sai.',
 'Notification Service sinh mã OTP sáu chữ số, lưu xuống cơ sở dữ liệu dưới dạng băm, hiệu lực năm phút, và gửi qua Email SMTP. Mã OTP được băm theo đúng nguyên tắc áp dụng cho mật khẩu: nếu cơ sở dữ liệu bị lộ thì kẻ tấn công vẫn không đọc được mã đang còn hiệu lực.',
 'Khách nhập họ tên, số điện thoại, mật khẩu và mã OTP, rồi gửi biểu mẫu. Hệ thống kiểm tra OTP theo ba điều kiện: đúng mã, chưa hết hạn, và chưa được sử dụng. Điều kiện thứ ba chống việc dùng lại cùng một mã nhiều lần trong năm phút đó.',
 'Sau đó hệ thống kiểm tra email chưa được đăng ký, và số điện thoại nếu có nhập thì cũng chưa được ai dùng. Cả hai đều phải duy nhất, vì cả hai đều là định danh đăng nhập hợp lệ.',
 'Cuối cùng hệ thống tạo bản ghi Account với mật khẩu đã băm bằng Bcrypt, và tạo bản ghi Customer liên kết với Account đó. Hai bản ghi này được ghi trong cùng một giao dịch cơ sở dữ liệu, vì một Account mà không có Customer thì khách không đặt lịch được và cũng không có chỗ lưu thống kê.',
...
SAY('SLIDE 8 → UC8 ĐĂNG KÝ XE (Nguyên · 80 giây)', [
 ...
 'Khách nhập OTP, hệ thống xác thực đúng ba điều kiện như ở đăng ký, rồi lưu bản ghi Vehicle với biển số đã được chuẩn hóa, và hiển thị thông báo MSG17.',
...
SAY('SLIDE 10 → UC12 TẠO ĐẶT LỊCH (Thuận · 110 giây)', [
 ...
 'Chặng thứ hai là kiểm tra. Hệ thống chạy năm điều kiện, và mỗi điều kiện chặn một loại rủi ro khác nhau.',
 ...
 'Cuối cùng, Notification Service tạo thông báo trong ứng dụng và gửi email xác nhận. Phần gửi email nằm ngoài giao dịch: nếu SMTP lỗi thì booking vẫn hợp lệ, hệ thống chỉ cần thử gửi lại email.',
])
...
SAY('SLIDE 13 → UC17, UC19, UC21 — CHECK-IN ĐẾN CHECK-OUT (Đạt · 120 giây)', [
 ...
 'Cuối cùng hệ thống tạo bản ghi Queue ở trạng thái Đang chờ, vị trí xếp cuối hàng. Thứ tự phục vụ tính theo thời điểm check-in, tức là đến trước phục vụ trước, chứ không theo giờ hẹn.',
 'Sang UC19 — Chuyển trạng thái rửa xe. Nhân viên chỉ bấm một nút Chuyển tiếp, hệ thống tự xác định công đoạn kế tiếp theo đúng sơ đồ chuyển trạng thái. Nhân viên không được chọn trạng thái tùy ý, để không nhảy cóc công đoạn và để dữ liệu thời gian sạch cho thống kê.',
 ...
 'Hệ thống ghi mốc hoàn tất, rồi lấy giá cuối đã chốt lúc đặt để chốt số tiền — không tính lại theo bảng giá hiện tại; đây chính là chỗ cột PriceSnapshot bạn Nguyên nói lúc nãy phát huy tác dụng.',
 ...
 'Kết thúc, Notification Service gửi thông báo và email hoàn tất cho khách.',
])
...
P('')
STEPS([
 ['1','Nhập email → bấm Gửi OTP.','Tách 2 bước để không bắt khách điền cả biểu mẫu rồi mới phát hiện email sai.'],
 ['2','Sinh OTP 6 số, lưu dạng băm, hiệu lực 5 phút, gửi qua SMTP (mẫu ET1).','Băm để database lộ cũng không đọc được mã còn hiệu lực; 5 phút thu hẹp cửa sổ tấn công.'],
 ['3','Nhập họ tên, SĐT, mật khẩu, OTP → gửi biểu mẫu.','—'],
 ['4','Kiểm tra OTP: khớp mã · chưa hết hạn · chưa dùng.','Điều kiện "chưa dùng" chống phát lại (replay) trong 5 phút.'],
 ['5','Email chưa được đăng ký.','Email là định danh đăng nhập nên phải duy nhất toàn hệ thống.'],
 ['6','SĐT (nếu có) chưa được dùng.','SĐT cũng là định danh đăng nhập hợp lệ ở UC1.'],
 ['7','Tạo Account, mật khẩu băm Bcrypt có salt.','Không lưu mật khẩu gốc; salt khiến 2 người cùng mật khẩu ra 2 hash khác nhau.'],
 ['8','Tạo Customer — CÙNG giao dịch với bước 7.','Account không có Customer thì không đặt lịch được, không có chỗ lưu thống kê.'],
 ['9','Chuyển sang màn hình đăng nhập hoặc tự đăng nhập.','—'],
])
...
STEPS([
 ...
 ['4','Sinh OTP 6 số (băm, 5 phút) gửi email đã đăng ký.','Xe là thứ dùng để check-in và nhận dịch vụ, phải chắc chắn chính chủ thao tác.'],
 ['5','Xác thực OTP (khớp · chưa hết hạn · chưa dùng).','Tái sử dụng nguyên cơ chế OTP của UC3.'],
...
STEPS([
 ...
 ['UC19','Bấm Chuyển tiếp → hệ thống tự xác định công đoạn kế tiếp; ghi StartedAt / CompletedAt.','Nhân viên không chọn trạng thái tùy ý — chống nhảy cóc và giữ dữ liệu thời gian sạch.'],
 ['UC21','Queue và Booking = Hoàn tất → ghi CompletedAt → lấy FinalPrice → cập nhật TotalVisits, TotalSpend, LastVisitAt → gửi email hoàn tất (ET3).','Đây là NƠI DUY NHẤT cập nhật thống kê khách hàng, và chỉ khi xe thực sự rửa xong.'],
])
TBL(['Thầy hỏi','Trả lời'],
[
